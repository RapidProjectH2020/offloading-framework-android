...
public class DSE {
  ...
  DSE(Context context, int userChoice, String dbName) {
  ...
  DSE(Context context, int userChoice) {
    this(context, userChoice, Constants.DEFAULT_DB_NAME);
  }
  ...
  public int findExecLocation(String appName, String methodName) {
    ...
    if (userChoice == Constants.LOCATION_LOCAL) {
      ...
      int ulRate = 1000 * 1000;
      int dlRate = 1000 * 1000;
  ...
  /**
   * Decide whether to execute remotely, locally, or hybrid. Use the DB.
   * 
   * @return The type of execution: LOCAL, REMOTE, HYBRID.
   * @deprecated Use the implementation that uses the DB cache instead.
   */
  public int findExecLocationDB(String appName, String methodName) {
    ...
    if (userChoice == Constants.LOCATION_LOCAL) {
      ...
      if (shouldOffloadDb(appName, methodName, ulRate, dlRate)) {
        Log.d(TAG, "Execute Remotely - True");
        return Constants.LOCATION_REMOTE;
      } else {
        Log.d(TAG, "Execute Remotely - False");
   ...
   *         <b>False</b> otherwise.
   * 
   * @deprecated This is much slower than using the DB cache implementation.
   */
  private boolean shouldOffloadDb(String appName, String methodName, int currUlRate,
      int currDlRate) {
    DatabaseQuery query = new DatabaseQuery(mContext, dbName);

    Log.i(TAG,
        "Trying to decide using normal DB where to execute the method: appName=" + appName
            + ", methodName=" + methodName + ", currUlRate=" + currUlRate + ", currDlRate="
            + currDlRate);
    ...
    Log.i(TAG, "nrLocalExec: " + nrLocalExec);
    Log.i(TAG, "meanDurLocal: " + meanDurLocal + "ns (" + meanDurLocal / 1000000000.0 + "s)");
    Log.i(TAG, "meanEnergyLocal: " + meanEnergyLocal + " mJ");
    ...
    // Log.d(TAG,
    // "meanEnergyRemote1: " + meanEnergyRemote1 + " meanEnergyRemote2: " + meanEnergyRemote2);
    Log.d(TAG, "meanEnergyRemote: " + meanEnergyRemote + " mJ");
